...
func CheckUpdate(version string) string {
	...
	if versionCompare(version, latest) {
		log.Infof("当前有更新的 study_xxqg 可供更新, 请前往 https://github.com/cccchenry/study_xxxxxx/releases 下载.")
...
func lastVersion() (string, error) {
	response, err := http.Get("https://api.github.com/repos/cccchenry/study_xxxxxx/releases/latest")
	if err != nil {
		return "", err
	}
...
func checksum(github, version string) []byte {
	sumURL := fmt.Sprintf("%v/cccchenry/study_xxxxxx/releases/download/%v/study_xxqg_checksums.txt", github, version)
...
func SelfUpdate(github string, version string) {
	...
	url := fmt.Sprintf("%v/cccchenry/study_xxxxxx/releases/download/%v/%v", github, latest, binaryName())
	if version == latest {
